...
@section('title', 'Редактировать озеленение')
...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Редактировать озеленение: {{ $post['name_green'] }}</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
            <a href="{{ route('ozelenenie.index') }}">Все озеленения</a>
            @if (session('success'))
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card card-primary">
                        <!-- form start -->
                        <form action="{{ route('ozelenenie.update', $post['id']) }}" method="POST"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ $post['name_green'] }}" name="name_green" class="form-control"
                                ...
                                <div class="form-group">
                                    <!-- select -->
                                    <div class="form-group">
                                        <label>Категория озеленения</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="coord_point" class="form-control" id="coord" placeholder="Выбрать координаты" value="{{$post['coord_point']}}" required>
                                </div>
                                ...
                                <div id="poligons" class="form-group">
                                    ...
                                    @foreach($post->poligons as $poligon)
                                        <div class="input-group input-group-sm" id="poligon_old{{ $loop->iteration }}">
                                            <label>Область ID:{{$poligon->id}}</label>
                                        <input type="text" name="coord_poligons_old[{{$poligon->id}}]" class="form-control col-8" value="{{$poligon->coord_poligon}}">

                                            <div class="input-group-append"><button type="button" data-poligon="{{$poligon->id}}" data-block="{{$loop->iteration}}" class="btn btn-danger btn-sm delpoligon_old"><i class="fas fa-trash"></i>Удалить</button></div>
                                        </div>
                                    @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="info" class="editor">{{ $post['info'] }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="@if(file_exists(storage_path('app/public/green/tumb/tumb-'.$post['img1']))) {{asset('public/storage/green/tumb/tumb-'.$post['img1'])}} @endif" alt="" class="img-uploaded"
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="@if(file_exists(storage_path('app/public/green/tumb/tumb-'.$post['img2']))) {{asset('public/storage/green/tumb/tumb-'.$post['img2'])}} @endif" alt="" class="img-uploaded"
                                         style="display: block; width: 300px">
                                    <div class="custom-file">
        ...
        function init() {
            ...
            var myPlacemark,
                myMap = new ymaps.Map('map', {
                    center: [{{$post['coord_point'] }}],
                    zoom: 16
                }, {
                    searchControlProvider: 'yandex#search'
                });

            myPlacemark = createPlacemark([{{$post['coord_point'] }}]);
            ...
            $('#addPolygon').click(
                ...
                function () {
                    ...
                    if( $(".form-control.col-8.new").length)
                        p = $(".form-control.col-8.new").length
                    else
                        p = 0;
            ...
            $('#dellPolygon').click(
                function () {
                    ...
                    r = $(".form-control.col-8.new").length;
                    $('#poligon_'+r).remove();
            ...
            function printGeometry (coords) {
                ...
                var r=1
                r += $(".form-control.col-8.new").length;
                $("#poligons").append('<div class="input-group input-group-sm" id="poligon_'+r+'"><input type="text" name="coord_poligons[]" class="form-control col-8 new" value="'+stringify(coords)+':'+color_polygon+'"></div>');
