            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card card-primary">
                        <!-- form start -->
                        <form action="{{ route('moy-biznes.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_rubric" class="form-control" required>
                                            @foreach ($categories as $category)
                                                <option value="{{ $category['id'] }}" @if(session('old.category_id')==$category['id']) selected @endif>{{ $category['title'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="@if(file_exists(storage_path('app/public/moy_biznes_image/'.session('old.img'))))
                                    {{asset('public/storage/moy_biznes_image/'.session('old.img'))}}
