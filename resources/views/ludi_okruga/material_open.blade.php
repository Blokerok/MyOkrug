...
@section('content')
    <div class="page-wrap">
        <div class="page-content">
            ...
            <div class="breadcrumbs"><a href="{{url('/')}}">Главная</a> / <a href="{{route('AllLudi')}}">"Моя история"</a> / {{$novost->h1}}</div>
            <h1>{{$novost->h1}}</h1>
            ...
            <div class="inits-card__descr">
                <figure class="img_news_1"><a href="{{asset('public/storage/moy_okrug_image/'.$novost->img)}}"
                                              class="fb"><img width="279"
                                                              src="@if($novost->img) {{asset('public/storage/moy_okrug_image/'.$novost->img)}} @else {{asset('public/uploads/news-03.jpg')}} @endif"/></a>
                </figure>
                ...
                <div class="gallery">
                    @foreach ($novost->images as $img)
                        <div class="item"><a data-fancybox="gallery"
                                    ...
                                    width="279" src="{{asset('public/storage/moy_okrug_image/'.$img->img)}}"/></a></div>
                    @endforeach
                </div>
            @endif
            ...
            <div class="inits-card__bottom-stat">
                <div class="objs-stat">
                    ...
                    <div class="objs-stat__item"><span class=" @if(count($novost->like))unmark_liked ico_liked @else mark_liked ico_like @endif" data-id="{{$novost->id}}" data-model="Ludi"> {{$novost->likes}} отметок «Нравится»</span></div>
            ...
            <p class="back"><a href="{{route('AllLudi')}}">Вернуться в раздел</a></p>
